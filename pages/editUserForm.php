...
<form action="<?php echo $targetURL ?>" method="post" name="editUser" onsubmit="return validateEditUser();">
    <fieldset>
        ...
        <?php echo translate("fname") ?> : <br><input type="text" name="fname" placeholder="Bob"
                                value="<?php echo $_COOKIE["fname"] ?? ""; ?>" autofocus required><br>
        <?php echo translate("lname") ?> :<br> <input type="text" name="lname" placeholder="Geldof"
        ...
        <?php echo translate("dob") ?>:<br> <input type="date" name="dob" placeholder="1974-01-31"
                             value="<?php echo $_COOKIE["dob"] ?? ""; ?>" required><br>

        <?php
            $userID = $_GET["userToEditID"];
        ?>
        <button onclick="saveUser(<?php echo $userID; ?>);" type="button" value="<?php echo $userID; ?>"><?php echo translate("save") ?></button>
    </fieldset>
</form>
